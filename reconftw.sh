...
function sub_noerror(){
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$SUBNOERROR" = true ]; then
		...
		if [[ $(echo "${RANDOM}thistotallynotexist${RANDOM}.$domain" | dnsx -r $resolvers -rcode noerror,nxdomain -retry 3 -silent | cut -d' ' -f2) == "[NXDOMAIN]" ]]; then 
			resolvers_update_quick_local
			if [ "$DEEP" = true ]; then
				dnsx -d $domain -r $resolvers -silent -rcode noerror -w $subs_wordlist_big -t 50 -rl 50 | cut -d' ' -f1 | anew -q .tmp/subs_noerror.txt 2>>"$LOGFILE" &>/dev/null
			else
				dnsx -d $domain -r $resolvers -silent -rcode noerror -w $subs_wordlist -t 50 -rl 50 | cut -d' ' -f1 | anew -q .tmp/subs_noerror.txt 2>>"$LOGFILE" &>/dev/null
			fi
		...
		if [ -s "subdomains/subdomains.txt" ]; then
			if [ "$NMAP_WEBPROBE" = true ]; then
				if [[ $domain =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9] ]]; then
					$SUDO nmap -iL subdomains/subdomains.txt -Pn -p $UNCOMMON_PORTS_WEB -oG .tmp/uncommon_nmap.gnmap 2>>"$LOGFILE" &>/dev/null
...
function nuclei_check(){
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$NUCLEICHECK" = true ]; then
		...
		if [ "$BBRF_CONNECTION" = true ]; then
			...
			[ -s "nuclei_output/critical.txt" ] && cat nuclei_output/critical.txt | cut -d' ' -f6 | sort -u | bbrf url add - -t 
nuclei:${crit} 2>>"$LOGFILE" &>/dev/null
		fi
	...
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$URL_CHECK" = true ]; then
		...
		if [ -s ".tmp/webs_all.txt" ]; then
			if [ ! "$AXIOM" = true ]; then
				...
				if [ $diff_webs != "0" ] || [ ! -s ".tmp/gau.txt" ]; then
					if [ "$URL_CHECK_ACTIVE" = true ]; then
						if [ "$DEEP" = true ]; then
							cat .tmp/webs_all.txt | gau --providers wayback,commoncrawl,otx,urlscan --o .tmp/gau.txt 2>>"$LOGFILE" &>/dev/null
						else
							cat .tmp/webs_all.txt | gau --providers wayback,commoncrawl,otx,urlscan --o .tmp/gau.txt 2>>"$LOGFILE" &>/dev/null
						fi
					fi
				...
				if [ $diff_webs != "0" ] || [ ! -s ".tmp/hakrawler.txt" ]; then
					if [ "$URL_CHECK_ACTIVE" = true ]; then
						if [ "$DEEP" = true ]; then
							cat .tmp/webs_all.txt | hakrawler -insecure > .tmp/hakrawler.txt 2>>"$LOGFILE" &>/dev/null
						else
							cat .tmp/webs_all.txt | hakrawler -insecure > .tmp/hakrawler.txt 2>>"$LOGFILE" &>/dev/null
						fi
					fi
				...
				if [ $diff_webs != "0" ] || [ ! -s ".tmp/waybackurls.txt" ]; then
					if [ "$URL_CHECK_ACTIVE" = true ]; then
						if [ "$DEEP" = true ]; then
							cat .tmp/webs_all.txt | waybackurls > .tmp/waybackurls.txt 2>>"$LOGFILE" &>/dev/null
						else
							cat .tmp/webs_all.txt | waybackurls > .tmp/waybackurls.txt 2>>"$LOGFILE" &>/dev/null
						fi
					fi
			...
			[ -s ".tmp/waybackurls.txt" ] && sed -i '/^.\{2048\}./d' .tmp/waybackurls.txt
			[ -s ".tmp/waybackurls.txt" ] && cat .tmp/waybackurls.txt | anew -q .tmp/url_extract_tmp.txt
			[ -s ".tmp/gau.txt" ] && sed -i '/^.\{2048\}./d' .tmp/gau.txt
			[ -s ".tmp/gau.txt" ] && cat .tmp/gau.txt | anew -q .tmp/url_extract_tmp.txt
			[ -s ".tmp/hakrawler.txt" ] && sed -i '/^.\{2048\}./d' .tmp/hakrawler.txt
			[ -s ".tmp/hakrawler.txt" ] && cat .tmp/hakrawler.txt | anew -q .tmp/url_extract_tmp.txt
...
function ssrf_checks(){
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$SSRF_CHECKS" = true ] && [ -s "gf/ssrf.txt" ]; then
		...
		if [ "$DEEP" = true ] || [[ $(cat gf/ssrf.txt | wc -l) -le $DEEP_LIMIT ]]; then
			...
			nuclei -l gf/ssrf.txt -t ~/nuclei-templates/fuzzing-templates/ssrf/ -o vulns/ssrf_nuclei.txt
			notification $(cat vulns/ssrf_nuclei.txt) info
			sleep 5
...
function crlf_checks(){
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$CRLF_CHECKS" = true ]; then
		...
		if [ "$DEEP" = true ] || [[ $(cat .tmp/webs_all.txt | wc -l) -le $DEEP_LIMIT ]]; then
			...
			nuclei -l .tmp/webs_all.txt -t ~/nuclei-templates/fuzzing-templates/crlf/ -o vulns/crlf_nuclei.txt
			notification $(cat vulns/crlf_nuclei.txt) info
			notification $(cat vulns/crlf_nuclei.txt) info
	...
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$LFI" = true ] && [ -s "gf/lfi.txt" ]; then
		...
		if [ -s "gf/lfi.txt" ]; then
			...
			if [ "$DEEP" = true ] || [[ $(cat .tmp/tmp_lfi.txt | wc -l) -le $DEEP_LIMIT ]]; then
				...
				nuclei -l .tmp/tmp_lfi.txt -t ~/nuclei-templates/fuzzing-templates/lfi/ -o vulns/lfi_nuclei.txt
				notification $(cat vulns/lfi_nuclei.txt) info
	...
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$SSTI" = true ] && [ -s "gf/ssti.txt" ]; then
		...
		if [ -s "gf/ssti.txt" ]; then
			...
			if [ "$DEEP" = true ] || [[ $(cat .tmp/tmp_ssti.txt | wc -l) -le $DEEP_LIMIT ]]; then
				...
				nuclei -l .tmp/tmp_ssti.txt -t ~/nuclei-templates/fuzzing-templates/ssti/ -o vulns/ssti_nuclei.txt
				notification $(cat vulns/ssti_nuclei.txt) info
...
function sqli(){
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$SQLI" = true ] && [ -s "gf/sqli.txt" ]; then
		...
		if [ "$DEEP" = true ] || [[ $(cat .tmp/tmp_sqli.txt | wc -l) -le $DEEP_LIMIT ]]; then
			...
			nuclei -l .tmp/tmp_sqli.txt -t ~/nuclei-templates/fuzzing-templates/sqli/ -o vulns/sqli_nuclei.txt
			notification $(cat vulns/sqli_nuclei.txt) info
...
function xxe(){
	#&& [ "$XXE" = true ]
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; }  ; then
		start_func ${FUNCNAME[0]} "XXE checks"

		
		# [[ $(cat .tmp/tmp_sqli.txt | wc -l) -le $DEEP_LIMIT ]]

		if [ "$DEEP" = true ]; then


		nuclei -l .tmp/url_extract_tmp.txt -t ~/nuclei-templates/fuzzing-templates/xxe/ -o vulns/xxe_nuclei.txt
		notification $(cat vulns/xxe_nuclei.txt) info

		fi
		##if [ "$SQLI" = false ]; then
		##	printf "\n${yellow} ${FUNCNAME[0]} skipped in this mode or defined in reconftw.cfg ${reset}\n"
		if [ ! -s ".tmp/url_extract_tmp.txt" ]; then
			printf "\n${yellow} ${FUNCNAME[0]} No URLs potentially vulnerables to XXE ${reset}\n\n"
		else
			printf "${yellow} ${FUNCNAME[0]} is already processed, to force executing ${FUNCNAME[0]} delete\n    $called_fn_dir/.${FUNCNAME[0]} ${reset}\n\n"
		fi
...
function command_injection(){
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$COMM_INJ" = true ] && [ -s "gf/rce.txt" ]; then
		...
		nuclei -l .tmp/tmp_rce.txt -t ~/nuclei-templates/fuzzing-templates/cmdi/ -o vulns/cmdi_nuclei.txt
		notification $(cat vulns/cmdi_nuclei.txt) info
	else
...
function smuggling(){
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$SMUGGLING" = true ] ; then
		...
		if [ "$DEEP" = true ] || [[ $(cat .tmp/webs_all.txt | wc -l) -le $DEEP_LIMIT ]]; then
			...
			python3 $tools/http-request-smuggling/smuggle.py -urls $dir/.tmp/webs_all.txt
			[ $(ls reports | wc -l) -gt $0 ] && notification "smuggling $(ls reports)" info
			cd "$dir" || { echo "Failed to cd to $dir in ${FUNCNAME[0]} @ line ${LINENO}"; exit 1; }
			[ -s ".tmp/smuggling.txt" ] && cat .tmp/smuggling.txt | anew -q vulns/smuggling.txt | notification $(cat vulns/smuggling.txt) info
...
function webcache(){
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$WEBCACHE" = true ] ; then
		...
		if [ "$DEEP" = true ] || [[ $(cat .tmp/webs_all.txt | wc -l) -le $DEEP_LIMIT ]]; then
			...
			cd "$dir" || { echo "Failed to cd to $dir in ${FUNCNAME[0]} @ line ${LINENO}"; exit 1; }
			[ -s ".tmp/webcache.txt" ] && cat .tmp/webcache.txt | anew -q vulns/webcache.txt | notification $(cat vulns/webcache.txt) info
